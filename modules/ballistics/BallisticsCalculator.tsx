...
export const BallisticsCalculator = () => {
        ...
        <div class="card">
          ...
          <div class="grid">
            <table>
              ...
              ${solutions.slice(0, Math.ceil(solutions.length / 2)).map(s => {
                const drop = (s as any)[dropKey];
                const drift = (s as any)[driftKey];
                const clicks = turretUnit === 'moa' ? Math.round(s.dropMOA * 4) : Math.round(s.dropMIL * 10);
                return `<tr><td>${s.range}</td><td>${drop}</td><td class="clicks">${clicks}</td><td>${drift}</td></tr>`;
              }).join('')}
            </table>
            <table>
              <tr><th>YDS</th><th>${unit}</th><th>CLICKS</th><th>WIND</th></tr>
              ${solutions.slice(Math.ceil(solutions.length / 2)).map(s => {
                const drop = (s as any)[dropKey];
                const drift = (s as any)[driftKey];
                const clicks = turretUnit === 'moa' ? Math.round(s.dropMOA * 4) : Math.round(s.dropMIL * 10);
                return `<tr><td>${s.range}</td><td>${drop}</td><td class="clicks">${clicks}</td><td>${drift}</td></tr>`;
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {selectedProfile && showReticleSim && (
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, color: '#94a3b8', fontSize: '0.8rem' }}>
                  Real-time trajectory drop and crosswind deflection projected onto a tactical optic reticle
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>Target Range:</span>
              <span style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#38bdf8', minWidth: '60px' }}>
                {simDistance} yds
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '0.8rem 1rem', borderRadius: '10px', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>Target Distance</span>
                <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#38bdf8' }}>{simDistance} Yards</span>
              </div>
              <input
                type="range"
                min={50}
            ...
            <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '0.8rem 1rem', borderRadius: '10px', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>Crosswind (Full 90° Value)</span>
                <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#fbbf24' }}>{simWindMph} MPH</span>
              </div>
              <input
                type="range"
                min={0}
          ...
          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center' }}>
            ...
            >
              <svg width="320" height="320" viewBox="0 0 320 320">
                ...
                {[-8, -6, -4, -2, 2, 4, 6, 8].map((unit) => {
                  ...
                  return (
                    <g key={`x-${unit}`}>
                      ...
                      {unit !== 0 && (
                        <text x={x} y="178" fill="#64748b" fontSize="8" fontFamily="monospace" textAnchor="middle">
                          {Math.abs(unit)}
                        </text>
                      )}
                    </g>
                ...
                {(() => {
                  ...
                  const driftVal = turretUnit === 'moa' ? simSolution.windDriftMOA : simSolution.windDriftMIL;
            ...
            {simSolution && (() => {
              const dropVal = turretUnit === 'moa' ? simSolution.dropMOA : simSolution.dropMIL;
              const driftVal = turretUnit === 'moa' ? simSolution.windDriftMOA : simSolution.windDriftMIL;
              const elevClicks = turretUnit === 'moa' ? Math.round(dropVal * 4) : Math.round(dropVal * 10);
              const windClicks = turretUnit === 'moa' ? Math.round(driftVal * 4) : Math.round(driftVal * 10);

              return (
                <div style={{ flex: 1, minWidth: '280px', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    <div style={{ background: 'rgba(56, 189, 248, 0.1)', border: '1px solid rgba(56, 189, 248, 0.3)', borderRadius: '10px', padding: '0.8rem' }}>
                      <div style={{ color: '#38bdf8', fontSize: '0.75rem', fontWeight: 'bold' }}>▲ ELEVATION HOLD</div>
                      <div style={{ color: '#fff', fontSize: '1.25rem', fontWeight: 'bold', fontFamily: 'monospace', margin: '2px 0' }}>
                    ...
                    <div style={{ background: 'rgba(251, 191, 36, 0.1)', border: '1px solid rgba(251, 191, 36, 0.3)', borderRadius: '10px', padding: '0.8rem' }}>
                      <div style={{ color: '#fbbf24', fontSize: '0.75rem', fontWeight: 'bold' }}>► WIND DEFLECTION</div>
                      <div style={{ color: '#fff', fontSize: '1.25rem', fontWeight: 'bold', fontFamily: 'monospace', margin: '2px 0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <div>
                      <div style={{ color: '#94a3b8', fontSize: '0.7rem' }}>TERMINAL VEL</div>
                      <div style={{ color: '#f8fafc', fontWeight: 600, fontSize: '0.9rem', fontFamily: 'monospace' }}>
              ...
              {(chronoStrings.length > 0 || opticsList.length > 0) && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.25rem' }}>
                  {chronoStrings.length > 0 && (
                    <div style={{ padding: '0.65rem', background: 'rgba(96, 165, 250, 0.08)', border: '1px solid rgba(96, 165, 250, 0.25)', borderRadius: '10px' }}>
                      <label style={{ display: 'block', color: '#60a5fa', fontSize: '0.78rem', fontWeight: 600, marginBottom: 4 }}>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (c) {
                            const avgVel = c.averageVelocity || c.average_velocity || (c.shotVelocities ? Math.round(c.shotVelocities.reduce((a: any, b: any) => a + b, 0) / c.shotVelocities.length) : null);
                            if (avgVel) {
                              setForm(prev => ({ ...prev, muzzleVelocity: avgVel, name: prev.name || c.ammoLabel || 'Chrono Load' }));
                            }
                          }
                        }}
                        defaultValue=""
                      >
                        <option value="" disabled>-- Pick Shot String --</option>
                        {chronoStrings.map((c, idx) => (
                          <option key={idx} value={idx}>
                            {c.ammoLabel || `String #${idx + 1}`} ({c.averageVelocity || c.average_velocity} fps)
                          </option>
                        ))}
                      </select>
                    </div>
                  ...
                  {opticsList.length > 0 && (
                    <div style={{ padding: '0.65rem', background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.25)', borderRadius: '10px' }}>
                      <label style={{ display: 'block', color: '#38bdf8', fontSize: '0.78rem', fontWeight: 600, marginBottom: 4 }}>
                        Link Mounted Optic:
                      </label>
                      <select
                        className="glass-input"
                        ...
                        defaultValue=""
                      >
                        <option value="" disabled>-- Select Optic Profile --</option>
                        {opticsList.map(o => (
      ...
      {isDopeModalOpen && selectedProfile &&
        createPortal(
          <div
            style={{
              position: 'fixed',
          ...
          >
            ...
            >
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.84rem', marginTop: 0, marginBottom: '1.25rem' }}>
                Select physical card format tailored for long-range field engagement, sniper stocks, or quarterback wrist sleeves.
              </p>
                ...
                {[
                  {
                    ...
                    icon: <Target size={18} color={dopeCardType === 'stock' ? '#38bdf8' : '#94a3b8'} />,
                  },
                  {
                    type: 'armband',
                    title: 'Forearm Sleeve',
                    desc: 'Narrow 3-col format for quarterback wrist playbooks',
                    icon: <Layers size={18} color={dopeCardType === 'armband' ? '#38bdf8' : '#94a3b8'} />,
                  },
                  {
                    type: 'standard',
                    title: 'Full Bench Sheet',
                    desc: 'Full trajectory sheet with energy, velocity, and flight time',
                    icon: <FileText size={18} color={dopeCardType === 'standard' ? '#38bdf8' : '#94a3b8'} />,
                  },
                ].map((fmt) => (
                  <div
                    key={fmt.type}
                    ...
                    style={{
                      ...
                      borderRadius: '10px',
                      cursor: 'pointer',
                      border: dopeCardType === fmt.type ? '2px solid #38bdf8' : '1px solid var(--border-light)',
                      background: dopeCardType === fmt.type ? 'rgba(56, 189, 248, 0.12)' : 'rgba(255, 255, 255, 0.02)',
                      transition: 'all 0.15s ease',
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: 4 }}>
                      {fmt.icon}
                      <span style={{ fontWeight: 'bold', fontSize: '0.85rem', color: dopeCardType === fmt.type ? '#38bdf8' : 'var(--text-primary)' }}>
                        {fmt.title}
                      </span>
                    </div>
              ...
              >
                <div style={{ fontWeight: 'bold', textAlign: 'center', borderBottom: '1px solid #cbd5e1', paddingBottom: 4, marginBottom: 6 }}>
                  PRINT PREVIEW: {dopeCardType.toUpperCase()} FORMAT
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: 4 }}>
                  <span>{selectedProfile.name} • {selectedProfile.caliber}</span>
                  <span>MV {selectedProfile.muzzleVelocity} fps | {turretUnit.toUpperCase()}</span>
                </div>
                <div style={{ fontSize: '0.75rem', color: '#475569', marginBottom: 8 }}>
                  Zero: {selectedProfile.zeroRange}y | Wind: {selectedProfile.windSpeed || 10}mph full value
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: '4px' }}>
                  {solutions.slice(0, 8).map((s) => (
                    <div key={s.range} style={{ background: '#e2e8f0', padding: '3px 6px', borderRadius: '4px', fontSize: '0.72rem' }}>
                      <strong>{s.range}y:</strong> +{turretUnit === 'moa' ? s.dropMOA : s.dropMIL} ({turretUnit === 'moa' ? Math.round(s.dropMOA * 4) : Math.round(s.dropMIL * 10)}c)
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
                ...
                >
                  <Printer size={15} /> Print {dopeCardType === 'stock' ? 'Stock Card' : dopeCardType === 'armband' ? 'Armband Sleeve' : 'DOPE Sheet'}
                </button>
              </div>
            </div>
          </div>,
